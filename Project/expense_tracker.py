...
class ExpenseTracker:
    def __init__(self, filename='expenses.json', api_key=None):
    ...
    def suggest_category_with_claude(self, description):
        """Use Claude AI to suggest a category based on description"""
        ...
        prompt = f"""Based on the expense description: "{description}"

Available categories: {', '.join(categories)}

Suggest the most appropriate category for this expense. Respond with only the category name."""
    ...
    def add_expense(self, amount, category, description, date=None, use_ai=False):
        """Add a new expense"""
        if date is None:
            date = datetime.now().strftime('%Y-%m-%d')

        if not category and use_ai:
            suggested = self.suggest_category_with_claude(description)
            if suggested:
                category = suggested
...
def main():
    api_key = None
    if CLAUDE_AVAILABLE:
        api_key = input("Enter your Anthropic API key (or press Enter to skip AI features): ").strip()
        if not api_key:
            api_key = None

    tracker = ExpenseTracker(api_key=api_key)
    ...
    while True:
        ...
        if CLAUDE_AVAILABLE and tracker.claude_client:
            print("7. Add Expense with AI Categorization")
            print("8. Exit")
        else:
            print("7. Exit")
        print("="*50)
        ...
        if choice == '1':
        ...
        elif choice == '7' and CLAUDE_AVAILABLE and tracker.claude_client:
            amount = input("Enter amount: ₹")
            description = input("Enter description: ").strip()
            date = input("Enter date (YYYY-MM-DD) or press Enter for today: ").strip()
            ...
            try:
                tracker.add_expense(amount, "", description, date, use_ai=True)
            except ValueError:
                print("Invalid amount. Please enter a number.")

        elif (choice == '7' and not (CLAUDE_AVAILABLE and tracker.claude_client)) or (choice == '8' and CLAUDE_AVAILABLE and tracker.claude_client):
